...
metadata:
  ...
  namespace: production
  annotations:
    iam.gke.io/gcp-service-account: myapp-app@project-id.iam.gserviceaccount.com
```

The pod uses the annotated Kubernetes ServiceAccount. GKE exchanges the K8s token for a GCP access token via Workload Identity -- no key files needed.
...
Enforce that only trusted container images run on GKE:
...
resource "google_binary_authorization_policy" "this" {
  ...
  default_admission_rule {
    ...
    require_attestations_by = [
      google_binary_authorization_attestor.this.name
    ]
  }
...
Prevent data exfiltration by defining a security perimeter around GCP services:

```hcl
resource "google_access_context_manager_service_perimeter" "this" {
  parent = "accessPolicies/${var.access_policy_id}"
  name   = "accessPolicies/${var.access_policy_id}/servicePerimeters/${var.name_prefix}"
...
VPC Service Controls block API calls from outside the perimeter, even if the caller has valid IAM permissions. Use for projects handling sensitive data.
